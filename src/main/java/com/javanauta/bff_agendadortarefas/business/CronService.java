...
public class CronService {
    ...
    @Scheduled(cron = "${cron.horario}") // anotacao do cron

    public void buscaTarefasProximaHora() {
        String token = login(converterParaRequestDTO());
        log.info("Iniciada a busca de tarefas");
        LocalDateTime horaFutura = LocalDateTime.now().plusHours(1); //  hora atual adicione mais 1 hora, no swagger tem que olocar mais 1 hora na aual
        LocalDateTime horaFuturaMaisCinco = LocalDateTime.now().plusHours(1).plusMinutes(5); // intervalo com 5 min
        //Qualquer tarefa que fique entre hor atual - e a hora futura + 1 hora
